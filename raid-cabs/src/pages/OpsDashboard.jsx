...
const TABS = [['overview','Overview',BarChart2],['pending','Pending Drivers',Car],['discounts','Discounts',CreditCard],['deposits','Deposits',Package],['drivers','Drivers',Car],['bookings','Bookings',Settings],['users','Users',Users]]
...
export default function OpsDashboard() {
  ...
  const load = useCallback(async () => {
    const [t,d,pd,b,dep,u] = await Promise.all([
      ...
      q(() => supabase.from('bookings').select('id,receipt_number,pickup_address,final_fare,status,created_at,user_id,driver_id,profiles(full_name),drivers(name)').order('created_at',{ascending:false}).limit(60)),
      q(() => supabase.from('deposits').select('id,amount,discount_applied,payment_ref,status,created_at,user_id,profiles(full_name,phone)').order('created_at',{ascending:false}).limit(60)),
                        ...
                        <td>
                          <div style={{ display:'flex', gap:4 }}>
                            <button className="ops-btn ops-btn-g"
                              onClick={async()=>{
                                await q(()=>supabase.from('drivers').update({is_approved:true}).eq('id',d.id))
