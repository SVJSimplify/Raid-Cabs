...
export default function DriverHome() {
  ...
  useEffect(() => {
    ...
    watchRef.current = navigator.geolocation.watchPosition(
      async pos => {
        ...
        setGpsError(false)
        // Push to Supabase max every 5 seconds
        const now = Date.now()
  ...
  const doAction = async (status, label) => {
    ...
    if (error) {
      ...
      toast.success(label)
      // Send email notifications (silent fail — non-critical)
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#05050e', fontFamily:"'Nunito',sans-serif", color:'#ede8d8' }}>
          ...
          <div className="dh-card" style={{ textAlign:'center', padding:'2.5rem 1.5rem' }}>
            ...
            {driverPos && isOnline && (
              <div style={{ marginTop:'1.25rem' }}>
                <LiveMap userPos={driverPos} showDrop={false} height={200}/>
              </div>
            )}
          </div>
        )}
        ...
        {currentBooking && (
          <>
            ...
            <div className="dh-card" style={{ padding:'1rem' }}>
              ...
              {mapExpanded && (
                <>
                  <LiveMap userPos={customerPos} driverPos={driverPos} height={300} liveLabel="Tracking live"/>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'.6rem', marginBottom:'.85rem' }}>
              {currentBooking.status === 'confirmed' && (
                <>
                  <button className="dh-btn dh-o" onClick={() => { setEnteredCode(''); setCodeModal(true) }} disabled={actionLoading}>
                    {actionLoading?<Spinner/>:<><Zap size={16}/> Start Trip — Enter Passenger Code</>}
                  </button>
      ...
      {/* Ride Code Verification Modal */}
      {codeModal && (
        <div className="sos-overlay" onClick={() => setCodeModal(false)}>
          <div style={{ background:'#0e0e20', border:'1px solid rgba(255,179,71,.25)', borderRadius:18, padding:'2rem', maxWidth:360, width:'100%', textAlign:'center' }} onClick={e => e.stopPropagation()}>
            <div style={{ fontSize:'2.2rem', marginBottom:'1rem' }}>🔑</div>
            <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:'1.15rem', color:'#ede8d8', marginBottom:'.5rem' }}>Enter Passenger Ride Code</h3>
            <p style={{ color:'#9890c2', fontSize:'.83rem', lineHeight:1.6, marginBottom:'1.5rem' }}>Ask the passenger for their 4-digit ride code before starting the trip</p>
            <input
              type="text" inputMode="numeric" maxLength={4} value={enteredCode}
              onChange={e => setEnteredCode(e.target.value.replace(/\D/g,'').slice(0,4))}
              placeholder="0000"
              style={{ width:'100%', background:'rgba(255,255,255,.05)', border:'1.5px solid rgba(255,179,71,.25)', borderRadius:10, padding:'.82rem 1rem', color:'#ede8d8', fontFamily:"'Playfair Display',serif", fontSize:'2.5rem', fontWeight:900, textAlign:'center', letterSpacing:'.3em', outline:'none', marginBottom:'1.25rem' }}
              autoFocus
            />
            <div style={{ display:'flex', gap:'.75rem' }}>
              <button
                disabled={enteredCode.length !== 4 || verifyingCode}
                onClick={async () => {
                  setVerifyingCode(true)
                  // Verify code against the user's profile
                  const { data: profile } = await supabase.from('profiles')
                    .select('ride_code').eq('id', currentBooking.user_id).maybeSingle()
                  if (!profile || profile.ride_code !== enteredCode) {
                    toast.error('Wrong code! Ask passenger to check their Ride Code in Profile → Safety tab.')
                  ...
                  setEnteredCode('')
                  setVerifyingCode(false)
                  doAction('in_progress', '🚗 Trip started!')
                }}
                style={{ flex:1, padding:'.82rem', background:'linear-gradient(135deg,#ffb347,#ff6b35)', color:'#05050e', border:'none', borderRadius:10, fontWeight:700, cursor:'pointer', fontFamily:"'Nunito',sans-serif", opacity: enteredCode.length!==4?.5:1 }}>
                {verifyingCode ? 'Checking…' : '✓ Verify & Start'}
              </button>
              <button onClick={() => setCodeModal(false)} style={{ padding:'.82rem 1.2rem', background:'transparent', border:'1px solid rgba(255,255,255,.1)', color:'#9890c2', borderRadius:10, fontWeight:700, cursor:'pointer', fontFamily:"'Nunito',sans-serif" }}>Cancel</button>
            </div>
            <p style={{ fontSize:'.72rem', color:'#504c74', marginTop:'1rem' }}>The passenger can find their code in Profile → Safety tab</p>
          </div>
        </div>
      )}
