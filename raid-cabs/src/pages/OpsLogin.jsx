...
export default function OpsLogin() {
  ...
  useEffect(() => {
    if (user && profile?.role === 'admin') {
      navigate(`/${OPS_PATH}/dashboard`, { replace: true })
    }
      ...
      <div style={{ minHeight:'100vh', background:'#03030a', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Nunito',sans-serif", padding:'2rem' }}>
        <div style={{ maxWidth:360, textAlign:'center' }}>
          ...
          <button onClick={() => signOut()}
            style={{ background:'transparent', border:'1px solid rgba(255,165,40,.25)', color:'#ffb347', borderRadius:8, padding:'.6rem 1.5rem', cursor:'pointer', fontFamily:"'Nunito',sans-serif", fontWeight:700, fontSize:'.88rem' }}>
            Sign Out
          </button>
        </div>
      </div>
      ...
      <div className="or">
        <div className="oc">
          <div style={{ textAlign:'center', marginBottom:'2rem' }}>
            ...
            <p style={{ color:'#504c74', fontSize:'.81rem', marginTop:'.3rem' }}>Restricted · Authorised personnel only</p>
          </div>

          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'1.15rem' }}>
            <div>
              <label className="ol-lbl">Email</label>
            ...
            <div>
              ...
              <div style={{ position:'relative' }}>
                ...
                <input className="oi" type={showPw?'text':'password'} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} required style={{ paddingLeft:'2.5rem', paddingRight:'2.8rem' }}/>
            ...
            <button type="submit" className="ob" disabled={loading} style={{ marginTop:'.3rem' }}>
              {loading ? <div className="os"/> : <><Shield size={16}/> Access Portal</>}
            </button>
          </form>
        </div>
      </div>
