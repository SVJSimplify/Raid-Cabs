...
import { getRouteInfo, searchPlaces, reverseGeocode, calcFare, searchCampusPlaces, CAMPUS_PLACES } from '../lib/location'
...
export default function BookCab() {
  ...
  const handlePickupInput = e => {
    ...
    if (!v.trim()) { setSugLoad(false); return }

    // Show campus places instantly (no API call)
    const campus = searchCampusPlaces(v)
    if (campus.length) setSugs(campus)

    if (v.length >= 2) {
      setSugLoad(true)
      sugTimer.current = setTimeout(async () => {
        const api = await searchPlaces(v)
        // Merge: campus first, then API results (deduplicated)
        const campusIds = new Set(campus.map(p => p.id))
        const merged = [...campus, ...api.filter(p => !campusIds.has(p.id))]
  ...
  const handleBook = async () => {
    ...
    toast.success(scheduled ? '⏰ Ride scheduled!' : '🚗 Cab booked! Track your driver.')
    // Navigate to dedicated tracking page — preserves state across refreshes
    await new Promise(r => setTimeout(r, 400))
    navigate('/active-booking/' + data.id, { replace: true })
                  ...
                  <div style={{position:'relative'}}>
                    ...
                    {(sugLoading||sugs.length>0)&&(
                      <div className="ac-list">
                        {!pickup && !sugLoading && (
                      <>
                        <div style={{padding:'.45rem 1rem',fontSize:'.7rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.07em',color:'var(--tm)',background:'rgba(255,255,255,.02)',borderBottom:'1px solid var(--b1)'}}>📍 Popular Locations</div>
                        {CAMPUS_PLACES.slice(0,5).map(p=>(
                          <div key={p.id} className="ac-item" onClick={()=>selectSug(p)}>
                            <MapPin size={13} style={{color:'var(--gold)',flexShrink:0}}/>
                            <div><div style={{fontWeight:600,fontSize:'.86rem'}}>{p.name}</div><div style={{fontSize:'.74rem',color:'var(--ts)',marginTop:1}}>{p.address}</div></div>
                          </div>
                        ))}
                      </>
                    )}
