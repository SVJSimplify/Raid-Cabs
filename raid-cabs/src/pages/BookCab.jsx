...
import { getRouteInfo, searchPlaces, reverseGeocode, calcFare } from '../lib/maps'
...
export default function BookCab() {
  ...
  const handleCancel = async () => {
    setCancelling(true)
    try {
      // Run both updates in parallel and wait for BOTH
      await Promise.all([
        bookingId
    ...
    setCancelling(false)
    // Small delay so DB write propagates before Dashboard re-fetches
  ...
  const FareCard = () => (
    <div className="card mb2">
      ...
      {[
        { l:'Distance',         v: `${routeInfo.distKm} km`,   c:'var(--tp)' },
      ...
      <p style={{ fontSize:'.71rem', color:'var(--tm)', marginTop:'.55rem' }}>
        ≈ Estimated road distance (straight-line × 1.4)
      </p>
    </div>
  )
      ...
      <div style={{ maxWidth:1080, margin:'0 auto' }}>
        ...
        {step === 'form' && (
          <>
            ...
            <div className="bk-grid">
                          ...
                          <div className="ac-list">
                            {sugLoad && (
                              <div className="ac-item" style={{ color:'var(--tm)', justifyContent:'center' }}>
                                <span className="spinner" style={{ width:15, height:15 }}/> Searching…
                              </div>
                            )}
                            {sugs.map(s => (
              ...
              <RideMap userPos={userPos} height={430}/>
            </div>
          </>
        )}
        ...
        {step === 'searching' && (
          <div style={{ textAlign:'center', padding:'7rem 2rem' }}>
            ...
            <p className="sub mt1">'Calculating route…'</p>
          </div>
        )}
