  ...
  const PwField = () => (
    <div className="fg">
      ...
      <div className="input-wrap" style={{ position:'relative' }}>
        ...
        <input className="input" type={showPw?'text':'password'} placeholder="Min. 6 characters" value={password} onChange={e=>setPassword(e.target.value)} required minLength={6} name="password" autoComplete={subMode==='login'?'current-password':'new-password'} style={{ paddingRight:'2.8rem' }}/>
        ...
        <div className="lcard">
          ...
          {mode==='phone' && (
            <>
              ...
              <form onSubmit={handlePhone} style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
                ...
                <div className="fg"><label className="label">Phone Number</label><div className="input-wrap"><Phone size={15} className="ico"/><input className="input" type="tel" inputMode="numeric" placeholder="98765 43210" value={phone} onChange={e=>setPhone(e.target.value)} required name="phone" autoComplete="tel"/></div><p className="hint">10-digit Indian mobile number</p></div>
                <PwField/>
          ...
          {mode==='email-otp' && (
            <>
              ...
              {otpStep==='input' && <form onSubmit={e=>{e.preventDefault();sendEmailOtp()}}><div className="fg mb3"><label className="label">Email Address</label><div className="input-wrap"><Mail size={15} className="ico"/><input className="input" type="email" placeholder="you@example.com" value={email} onChange={e=>setEmail(e.target.value)} required autoFocus name="email" autoComplete="email"/></div></div><button type="submit" className="btn btn-primary btn-blk btn-lg" disabled={loading}>{loading?<span className="spinner"/>:<><ArrowRight size={16}/> Send Code</>}</button></form>}
